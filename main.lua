...
function love.load()
    ...
    player1a = Paddle(20, VIRTUAL_HEIGHT / 2, 5, 20)
    player1b = Paddle(VIRTUAL_WIDTH - 20, VIRTUAL_HEIGHT / 2, 5, 20)

    player2a = Paddle(VIRTUAL_WIDTH / 2 - 10, 20, 20, 5)
    player2b = Paddle(VIRTUAL_WIDTH / 2 - 10, VIRTUAL_HEIGHT - 20, 20, 5)
...
function love.update(dt)
    if gameState == "serve" then
        ...
        ball.dx = math.random(50, 100) * xdir
        ball.dy = math.random(50, 100) * ydir
    elseif gameState == "play" then
        -- detect ball collision with paddles, reversing dx if true and
        -- slightly increasing it, then altering the dy based on the position
        -- at which it collided, then playing a sound effect
        if ball:collides(player1a) then
        ...
        if ball:collides(player2a) then
            ball.dy = -ball.dy * 1.03
            ball.y = player2a.y + 5

            -- keep velocity going in the same direction, but randomize it
            if ball.dx < 0 then
                ball.dx = -math.random(50, 100)
            else
                ball.dx = math.random(50, 100)
            end
    ...
    if love.keyboard.isDown("w") then
        player1a.dy = -PADDLE_SPEEDY
        player1b.dy = -PADDLE_SPEEDY
    ...
    if love.keyboard.isDown("left") then
        player2a.dx = -PADDLE_SPEEDX
        player2b.dx = -PADDLE_SPEEDX
    ...
    -- AI top/bottom
    if player2a.x > ball.x then
        player2a.dx = -PADDLE_SPEEDX
        player2b.dx = -PADDLE_SPEEDX
    elseif player2a.x + player2a.width < ball.x then
        player2a.dx = PADDLE_SPEEDX
        player2b.dx = PADDLE_SPEEDX
    else
        player2a.dx = 0
